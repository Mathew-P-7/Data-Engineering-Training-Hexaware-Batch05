...
SELECT p.name AS pet_name, p.type, v.issue AS visit_issue
FROM Pets p
JOIN Visits v
ON p.pet_id = v.pet_id;

-- 4. Show the total number of visits per pet.
SELECT p.pet_id, p.name, COUNT(v.visit_id) AS total_visits
FROM Pets p
JOIN Visits v
ON p.pet_id = v.pet_id
GROUP BY p.pet_id;
...
SELECT type, AVG(age) AS average_pet_age
FROM Pets
GROUP BY type;
...
-- inserting few more data into the Pets table according to the query
INSERT INTO Pets VALUES
(7, 'Fluffy', 'Rabbit', 'Angora', 2, 'Priya'),
(8, 'Goldie', 'Fish', 'Goldfish', 1, 'Vikram');

SELECT p.pet_id, p.name
FROM Pets p
WHERE p.pet_id NOT IN (SELECT v.pet_id FROM Visits v);
